...
import {
  ...
  Navigation, RefreshCw, FileText, Camera, Sparkles, User, Star
} from 'lucide-react';
              ...
              <div className="p-6">
                ...
                {trackingLoading && !tracking ? (
                  ...
                  <>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="space-y-1 text-sm text-gray-600">
                              <p>
                                Latitud: {tracking.current_location.latitude?.toFixed(6) || 'N/A'}
                              </p>
                              <p>
                                Longitud: {tracking.current_location.longitude?.toFixed(6) || 'N/A'}
                              </p>
                              {tracking.current_location.last_update && (
                                <p className="text-xs text-gray-500 mt-2">
                                  Actualizado: {new Date(tracking.current_location.last_update).toLocaleTimeString()}
                                </p>
                              )}
                            </div>
                          </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4">
                        ...
                        <div className="card text-center bg-purple-50">
                          ...
                          <p className="text-2xl font-bold text-purple-900">
                            {tracking.eta.distance_km?.toFixed(1) || 'N/A'}
                          </p>
                    ...
                    {/* Live Tracking Map */}
                    {tracking?.current_location && selectedRequest.location_latitude && selectedRequest.location_longitude ? (
                      <div className="mb-4">
                        <TrackingMap
                          userLocation={{
                          ...
                          techLocation={{
                            lat: tracking.current_location.latitude || 14.6349,
                            lng: tracking.current_location.longitude || -90.5069
                          }}
                          techName={tracking.provider?.driver_name || 'Tecnico'}
                          eta={tracking.eta?.eta_minutes}
                          autoRefresh={!['COMPLETED', 'CANCELLED'].includes(selectedRequest.status)}
                          onRefresh={() => loadTracking(selectedRequest, true)}
                        />
                      </div>
                    ) : (
                      <div className="h-48 bg-gradient-to-br from-blue-100 to-purple-100 rounded-xl flex items-center justify-center mb-4 border-2 border-dashed border-blue-300">
                        <div className="text-center text-blue-700">
                          <MapPin size={32} className="mx-auto mb-2" />
                          <p className="font-medium">Vista de Mapa</p>
                          <p className="text-xs text-blue-600">Esperando ubicacion del tecnico...</p>
                        </div>
                      </div>
                    )}
                            ...
                            return (
                              <div key={step.status} className="relative flex items-start gap-4">
                                ...
                                <div className="flex-1 pb-6">
                                  ...
                                  {current && step.status === 'EN_ROUTE' && tracking?.eta?.eta_minutes && (
                                    <div className="mt-2 flex items-center gap-2 text-blue-700">
                                      <Clock size={16} />
                                      <span className="text-sm font-medium">
                                        Llegada estimada en {tracking.eta.eta_minutes} minutos
                                      </span>
                                    </div>
                                  )}
                                </div>
